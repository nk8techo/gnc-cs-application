...
WHERE (((qurReportingBase.Category)="N") AND ((qurReportingBase.[Project Status])="A") AND ((Year([Open]))=Year(Date())) AND ((qurReportingBase.[Corporate or Franchise])="C") AND ((qurReportingBase.[Open-Close])=True))
